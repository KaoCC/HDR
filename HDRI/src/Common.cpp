...
auto constructRadiance(const std::vector<HDRI::RawImage> &imageFiles, const std::array<cv::Mat, 3> &gCurves,
                       HDRI::WeightFunction &dwf, const std::vector<double> &expo) -> cv::Mat {
  ...
  for (auto idx = 0; idx < gCurves.size(); ++idx) { // r, g, b
    for (auto y = 0; y < height; ++y) {
      for (auto x = 0; x < width; ++x) {
        ...
        // loop over all imgs
        for (auto k = 0U; k < imageFiles.size(); ++k) {

          auto pixelColor = static_cast<int>(imageFiles[k].getImageData().at<cv::Vec3b>(y, x)[idx]);
...
auto convertRGB(const float r, const float g, const float b) -> float {
