...
auto HDRI::ReinhardAlgo::toneMap(const cv::Mat &inputRadiance) -> cv::Mat {
  ...
  for (auto y = 0U; y < inputRadiance.size().height; ++y) {
    for (auto x = 0U; x < inputRadiance.size().width; ++x) {
      ...
      lumi.at<float>(y, x) = convertRGB(inputRadiance.at<cv::Vec3f>(y, x)[2], inputRadiance.at<cv::Vec3f>(y, x)[1],
                                        inputRadiance.at<cv::Vec3f>(y, x)[0]);
    }
  }
